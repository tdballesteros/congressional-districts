...
tract_data <- read.csv("Data/oh_vote_merged_final.csv")
...
pop <- oh2010 %>%
  ...
  dplyr::full_join(tract_data, by = "GEOID10") %>%
  dplyr::mutate(
...
pop_tracts <- unique(pop$GEO_ID)
adj_tracts <- unique(adj$SOURCE_TRACTID)

pop_tracts[pop_tracts %!in% adj_tracts]
adj_tracts[adj_tracts %!in% pop_tracts]
...
splitIntoTwo <- function(df = pop, adjdf = adj){
  ...
  tdf <- dplyr::left_join(tdf, df, by=c("GEO_ID")) %>%
